...
function formatSessionTimeLabel(session: { start_timestamp?: string | null; end_timestamp?: string | null; time?: string | null }, timePrefix = 'Time: '): string | null {
...
export default function TrainingSessionPanel({ coachId }: Props) {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
      <Text style={{ fontSize: 15, fontWeight: '700', marginTop: 10, marginBottom: 8, color: tc.textPrimary }}>{t('TS_PANEL_TITLE')}</Text>

      {disabled && (
        <View style={{ backgroundColor: tc.bgSurface, borderRadius: 12, padding: 14 }}>
          <Text style={{ fontWeight: '700', fontSize: 14, marginBottom: 4, color: tc.textPrimary }}>{t('COMMON_ERR_NOT_SIGNED_IN')}</Text>
          <Text style={{ color: tc.textSecondary }}>{t('TS_PANEL_SIGN_IN_MSG')}</Text>
        </View>
      )}

      {!!(sessionsError || sessionsQuery.isError) && (
        <View style={{ padding: 14, borderRadius: 14, backgroundColor: tc.bgSurface, borderWidth: 1, borderColor: '#FECACA', marginBottom: 12 }}>
      ...
      {(sessionsQuery.isLoading || sessionsLoading) ? (
        ...
        <View style={{ backgroundColor: tc.bgSurface, borderRadius: 12, padding: 14 }}>
          <Text style={{ fontWeight: '700', fontSize: 14, marginBottom: 4, color: tc.textPrimary }}>{t('TS_PANEL_EMPTY_TITLE')}</Text>
          <Text style={{ color: tc.textSecondary }}>{t('TS_PANEL_EMPTY_MSG')}</Text>
          <TouchableOpacity
                ...
                >
                  ...
                  >
                    ...
                    <View style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        {t('EVENT_PANEL_PARTICIPANTS_PREFIX')}{s.numberofpeople ?? 0}/{s.participants_cap ?? '-'}
                      </Text>
                    </View>
                </TouchableOpacity>
              </View>
      ...
      {sessions.length > 0 && selectedSessionId != null && (
        <>
          ...
          {bookingsError && <Text style={{ color: 'red', marginBottom: 8 }}>{t('EVENT_PANEL_APPLICANT_LOAD_ERROR')}{bookingsError}</Text>}
          ...
          <View style={{ backgroundColor: tc.bgSurface, borderRadius: 12, padding: 14 }}>
            ...
            {blockedLoading ? (
              ...
              <View style={{ marginTop: 8 }}>
                {blocked.map((b) => (
                  <View key={b.blockid} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: tc.divider }}>
                    <TouchableOpacity
                      activeOpacity={0.75}
          ...
          >
            <Text style={{ fontSize: 15, fontWeight: '700', color: tc.textPrimary }}>{t('TS_PANEL_MODIFY_TITLE')}</Text>
            <TouchableOpacity
              activeOpacity={0.75}
          ...
          <View style={{ backgroundColor: tc.bgSurface, borderRadius: 12, padding: 12 }}>
            {infoLoading ? (
              ...
              <>
                ...
                >
                  <Text style={{ color: '#fff', fontWeight: '700' }}>{saving ? t('EVENT_PANEL_BTN_SAVING') : t('EVENT_PANEL_BTN_SAVE')}</Text>
                </TouchableOpacity>
                ...
                >
                  <Text style={{ color: '#fff', fontWeight: '700' }}>{cancellingSession ? t('TS_PANEL_BTN_CANCELLING_SESSION') : t('TS_PANEL_BTN_CANCEL_SESSION')}</Text>
                </TouchableOpacity>
              </>
            )}
          </View>
