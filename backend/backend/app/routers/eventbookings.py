...
router = APIRouter(prefix="/eventbookings", tags=["bookings"])
...
def list_event_bookings(userid: int | None = Query(None), status: str | None = Query(None), limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0)):
    try:
        ...
        data = rest_select("eventbookings", "*", filters=filters or None, order={"column": PRIMARY_KEY})
...
def get_event_booking(eventbookingid: int):
    try:
        row = rest_select("eventbookings", "*", filters={PRIMARY_KEY: eventbookingid}, single=True)
        if not row:
...
def create_event_booking(body: dict, current_user: str = Depends(get_current_user)):
    try:
        ...
        resp = rest_upsert("eventbookings", payload)
