...
load_dotenv()  # fallback to project root .env if needed
...
def get_settings() -> Settings:
    ...
    if not s.SUPABASE_URL or not s.SUPABASE_SERVICE_ROLE_KEY:
        raise RuntimeError("Supabase settings missing. Ensure SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are set.")
    return s
...
def rest_select(table: str, select: str, filters: Optional[Dict[str, Any]] = None, single: bool = False, order: Optional[Dict[str, Any]] = None) -> Any:
    ...
    if filters:
        for k, v in filters.items():
            params[k] = f"eq.{v}"  # simple equality; extend as needed
    if order:
